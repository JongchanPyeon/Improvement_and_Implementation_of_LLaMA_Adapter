...
class LLaMA_adapter(nn.Module):
    ...
    def clip_encode_image(self, x):
        ...
        x = x.to(device)  # Ensure the image tensor is on the correct device
        x = self.clip.visual.conv1(x)
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)
        x = torch.cat([self.clip.visual.class_embedding.to(x.dtype).to(device) +
                      torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=device), x], dim=1)
        x = x + self.clip.visual.positional_embedding.to(x.dtype).to(device)
        x = self.clip.visual.ln_pre(x)
        x = x.permute(1, 0, 2)
        x = self.clip.visual.transformer(x)
        x = x.permute(1, 0, 2)
        x = self.clip.visual.ln_post(x[:, :, :])
        if self.clip.visual.proj is not None:
            x = x @ self.clip.visual.proj.to(device)
...
#     @torch.inference_mode()
#     def generate(self, imgs, prompts, max_gen_len=256, temperature=0.1, top_p=0.75):
...
#         return decoded
    ...
    def generate(self, imgs, prompts, max_gen_len=256, temperature=0.1, top_p=0.75):
        ...
        total_len = max_prompt_length + max_gen_len  # Total sequence length

        # Debug print for verification
        print(f"Total length: {total_len}, Max prompt length: {max_prompt_length}")
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            tokens[:, cur_pos] = next_token
            # Early stopping for batch size of 1
            if bsz == 1 and next_token[0] == self.tokenizer.eos_id:
                break
            prev_pos = cur_pos
